...
from config                         import ancho_columnas, ESTADOS, FONT_DATOS, SIZE_FONT_DATOS, SIZE_FONT_TITULO, TITULO, EXTENSION_EXCEL_365, ESTADO_COLORES
...
def color_rows(row, limite_rap_para_normalizar: int):
    try:
        color = ESTADO_COLORES[row["estado"]]
    except:
        ...
        por_evaluar        = pd.to_numeric(row[col_aprobado], errors='coerce')
        juicios_productiva = pd.to_numeric(row[col_productiva], errors='coerce')
...
def write_process_file(folder: str, ficha: str, df_datos: pd.DataFrame, df_novedades_ficha: pd.DataFrame, df_activos_ficha: pd.DataFrame, df_hoja: pd.DataFrame):
    ...
    try:
        with pd.ExcelWriter(os.path.join(folder, file_name), engine="openpyxl") as writer:
            ...
            # with workbook[HOJA_DATOS] as worksheet:
            ...
            #     for cell in [worksheet[2], worksheet[3]]:
            ...
            # with workbook[HOJA_HOJA] as worksheet:
