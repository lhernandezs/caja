...
class FiltrosHelper():
    def procesar(self, df_datos: pd.DataFrame) -> dict:
        ...
        limite_rap_para_normalizar = int(np.ceil((df_datos.at[2, "aprobado"] + df_datos.at[2,"porEvaluar"] + df_datos.at[2,"noAprobado"]) * PORCENTAJE_RAPS_POR_NORMALIZAR / 100 ))
        ...
        df_error_productiva = df_en_formacion[(pd.to_numeric(df_en_formacion['porEvaluar'], errors='coerce') == 1) &
                                              (df_en_formacion.iloc[:, columna_productiva] != 1)].reset_index(drop=True) 
        df_para_normaliar   = df_en_formacion[(pd.to_numeric(df_en_formacion['porEvaluar'], errors='coerce').between(2, limite_rap_para_normalizar - 1))].reset_index(drop=True)
        ...
        for index in df_para_normaliar.index:
            for col_competencia in range(10, 24):
                sufijo_competencia  = f"{df_datos.columns[col_competencia]}"
                if (df_para_normaliar.iloc[index, col_competencia] > 0) and (df_datos.columns[col_competencia] != "PRO"):
                    instructor          = f"{df_datos.iloc[0, col_competencia]}"
                    nombres             = f"{df_para_normaliar.iloc[index, 2]}"
                    apellidos           = f"{df_para_normaliar.iloc[index, 3]}"                    
                    rap_por_evaluar     = f"{df_para_normaliar.iloc[index, col_competencia]}"
        ...
        return {
            ...
            'df_para_normaliar'     : df_para_normaliar,            
    ...
    def calular_raps_programa(self, df_datos: pd.DataFrame) -> int:
        return len(df_datos['juicios'].nunique())
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(resultados[df_name])            # linea = ""
            # for item in value:
            #     linea += f"{item:30} "
            # print(linea)
